...
500 kV network construction and scenario pipeline for PyPSA. Steps 01–19 build
and dispatch the operational 2024 network. Steps 20A–22 simplify, cluster and
build/optimize future-year scenarios on top of that base.
...
| `20A_simplify_network.py` | Collapses secondary buses to their 500 kV parents and exports a simplified PyPSA network with all 2024 generators, loads and profiles attached | `network_500kv.nc` + `buses_final.csv` + `fusion_map.csv` + `generators_2024.csv` + `loads_2024.csv` + `marginal_costs_2024.csv` + `gen_profiles_2024.csv` (external) | `network_500kv_simplified.nc` |
| `20B_network_clustering.py` | Spatial k-means clustering of the simplified network into K regions | `network_500kv_simplified.nc` | `cluster_k{N}.nc` + `cluster_summary_k{N}.csv` + `clusters_k{N}.gpkg` |
| `21_build_scenario.py` | Builds a future-year scenario (2035 by default) on top of a clustered network: scales demand, applies time aggregation (TSAM typical days), adds expandable RES/thermal generators with ATB costs, recomputes marginal costs of existing thermal units with target-year fuel prices and real CAMMESA efficiencies, makes inter-cluster lines expandable, and adds load shedding | `cluster_k{N}.nc` + `costs_2035_US.csv` (external) + `fuel_properties.yaml` (external) + `carrier_defaults.yaml` (external) | `scenario_<name>_k{N}.nc` |
| `22_run_scenario.py` | Solves the joint capacity expansion + dispatch LP on a scenario network and produces summary CSVs (global, by carrier, by cluster, by line, by fuel, new capacity, fuel by generator) plus the post-optimization `.nc` | `scenario_<name>_k{N}.nc` + `cluster_names_k{N}.csv` + `fuel_properties.yaml` (external) | `results_<name>_k{N}/` directory |
...
Builds `marginal_costs_2024.csv` with a fixed annual marginal cost per generator
unit, plus three machine-level efficiency-related columns used by scripts 20A
and 21. Requires `marginal_costs_diagnostic_completed.csv` to be completed
(generated by 18).
...
**Efficiency lookup (CAMMESA `efficencies.xlsx`):**
Two lookups are built and applied in this order to populate the
`efficiency`, `heat_rate_kcal_per_kwh`, `efficiency_fuel` and
`efficiency_source` columns:

1. **Combined-cycle inheritance (`CC_INHERITED`).** When the unit name (e.g.
   `ACAJTG02`) starts with a 4-letter plant prefix that has at least one
   `XXXXCC??` record in the efficiency table, the unit inherits the CC's
   efficiency, heat rate and fuel. This reflects that in Argentina TG and
   TV units of a combined-cycle plant always operate together as a CC; the
   per-machine table reports each TG with its (lower) open-cycle efficiency
   only because CAMMESA reports them as separate equipment, but they never
   run alone in dispatch.
2. **Direct match (`DIRECT`).** Otherwise, exact match by `bus_name_origen`
   against the `GENERADOR` column of the efficiency table.

When multiple CC records exist for a plant with different values (a few plants
have e.g. `CC25` and `CC26` with slightly different efficiencies) the most
efficient one (lowest heat rate) is picked. Fuel hierarchy is GN > FO > GO.
Coal (CM), uranium (U2/UA/UE) and other fuels are ignored — they don't go
through this path.
...
Collapses every secondary (low-voltage) bus to its 500 kV parent using
`fusion_map.csv`, removes the now-redundant transformers, and produces a clean
PyPSA network exported to `network_500kv_simplified.nc`. Generators, loads,
hourly availability profiles and marginal costs are attached so the resulting
file is the single self-contained input for clustering and scenarios.

When loading generators, this script also reads from `marginal_costs_2024.csv`
the per-unit `efficiency`, `heat_rate_kcal_per_kwh` and `efficiency_fuel`
columns produced by 18b. The `efficiency` value is assigned to the PyPSA
Generator's native `efficiency` attribute; the heat rate and fuel code are
stored as custom columns on `n.generators` so they survive clustering and are
available later in scripts 21 and 22.

The script runs an integrity validation block at the end (component balance,
referential integrity, topological connectivity, special cases) and exits with
status `OK`, `WARN` or `FAIL`. The two known disconnected single-bus islands
(`T PEPE`, `PBUENA2`) trigger a `WARN` and are removed by 20B.

**Configurable parameters:**
- `EXCLUDE_NO_COST` — False (default): keep generators with no marginal_cost (cost=0). True: exclude.

**Output:** `networks/network_500kv_simplified.nc` (not versioned in git).

---

### `20B_network_clustering.py`
Spatial k-means clustering of the simplified network into K regions. Operates
on `network_500kv_simplified.nc` and reduces the bus count from ~80 to K
(typical: 10, 20, 30). Generators inherit the cluster bus, lines are aggregated,
and time series are preserved.

Before clustering, isolated single-bus islands (`T PEPE`, `PBUENA2`) and the
international connector buses (`BRASIL`, `GARABI`) are removed so the final
network is 100% Argentine and fully connected.

**Configurable parameters:**
- `CLUSTER_SIZES` — list of K values to produce (default: `[10, 20, 30]`)
- `BUS_WEIGHTING` — `uniform` / `p_nom` / `demand` (default: weighted by demand + p_nom)
- `BUSES_TO_REMOVE` — set of bus names dropped before clustering

**Outputs** in `data/network_500kv/clusters/`:
- `cluster_k{N}.nc` — clustered network ready for scripts 21/22 (not versioned)
- `cluster_summary_k{N}.csv` — installed capacity by carrier by cluster
- `clusters_k{N}.gpkg` — buses, centroids and inter-cluster lines for QGIS

---

### `21_build_scenario.py`
Builds a future-year scenario on top of a clustered network. The default
configuration produces a 2035 BAU scenario for K=10. The script does, in order:

1. Loads `cluster_k{N}.nc` and scales hourly demand by `(1+r)^N_years`
   (default: 3% annual for 11 years → factor 1.384).
2. Time aggregation via TSAM. Per-cluster demand and per-(carrier, cluster)
   weighted-average availability profiles (~50–70 columns) are aggregated into
   16 typical days × 24 h = 384 snapshots using hierarchical clustering and the
   `distributionAndMinMaxRepresentation` method. The original `p_max_pu` of
   each generator is reassigned from its (carrier, cluster) typical profile so
   hourly availability constraints are preserved (especially the 2024 hydro
   dispatch ceiling).
3. Loads ATB-US 2035 cost data (`costs_2035_US.csv`) for new builds. WACC
   defaults to ATB values (~5%); a single `WACC_OVERRIDE` is available.
4. Adds expandable generators (one per carrier per cluster) for `ccgt`, `ocgt`,
   `diesel`, `solar`, `wind`. Capital cost is annualized with each carrier's
   ATB lifetime + WACC. Marginal cost for thermal new builds is computed as
   `VOM + fuel_price_2035 / efficiency`. New solar/wind get the (carrier,
   cluster) typical profile so they generate where there is resource. New
   thermal generators are tagged with their fuel code (CCGT/OCGT → `GN`,
   diesel → `GO`).
5. Per-cluster expansion caps for variable RES are enforced via `p_nom_max`
   (default: 5,000 MW solar / 3,000 MW wind total, distributed uniformly across
   K clusters; see `EXPANSION_LIMITS_MW`).
6. **Recomputes `marginal_cost` for every existing thermal generator** using
   target-year fuel prices, the real CAMMESA efficiency loaded by 18b/20A, and
   ATB VOM. Existing units without a CAMMESA match use ATB efficiency as
   fallback and the per-carrier default fuel code (CCGT/OCGT → `GN`,
   diesel → `GO`, steam → `FO`). Defaults and fallback efficiencies are read
   from `carrier_defaults.yaml`. Fuel prices come from the 2035 figures Tavo
   provided (`GAS_PRICE_USD_MWH_TH`, `DIESEL_PRICE_USD_MWH_TH`,
   `FUEL_OIL_PRICE_USD_MWH_TH`).
7. Makes existing inter-cluster lines expandable. Capital cost uses overhead
   HVAC values from ATB / Danish Energy Agency Technology Data (Jul 2025),
   annualized with a 40-year lifetime and the 2.7% transmission WACC fallback.
8. Adds a virtual load-shedding generator at each cluster
   (`LOAD_SHEDDING_COST_USD_MWH = 5,000`) to guarantee feasibility.

**Configurable parameters:**
- `SCENARIO_NAME`, `K`, `BASE_YEAR`, `TARGET_YEAR`, `DEMAND_GROWTH_RATE`
- `TIME_AGGREGATION` (`tsam` / `uniform` / `none`), `TSAM_TYPICAL_PERIODS`,
  `TSAM_HOURS_PER_PERIOD`, `TSAM_CLUSTER_METHOD`, `TSAM_REPRESENTATION`
- `WACC_OVERRIDE`, `EUR_TO_USD`
- `GAS_PRICE_USD_MWH_TH`, `DIESEL_PRICE_USD_MWH_TH`, `FUEL_OIL_PRICE_USD_MWH_TH`
- `EXPANSION_LIMITS_MW` — per-cluster cap for new RES
- `LOAD_SHEDDING_COST_USD_MWH`
- HVAC transmission expansion parameters

**Output:** `networks/scenarios/scenario_<SCENARIO_NAME>_k{N}.nc` (not versioned).

---

### `22_run_scenario.py`
Solves the joint capacity-expansion + dispatch LP on a scenario network and
produces a results directory with summary CSVs and the post-optimization `.nc`.

The optimization horizon is the entire year (8,784 hours via TSAM weights).
The default solver is HiGHS (open source, bundled with PyPSA). Gurobi and
CPLEX are accepted via `--solver` if installed.

**Outputs** in `networks/scenarios/results_<SCENARIO_NAME>_k{N}/`:
- `results_<SCENARIO_NAME>_k{N}.nc` — full PyPSA network with optimal capacities,
  hourly dispatch, line flows and marginal prices.
- `summary_global.csv` — one-row scenario KPIs (demand, generation, RES share,
  load shedding, cost, year factor).
- `summary_by_carrier.csv` — annual generation, existing/new/total capacity,
  capacity factor, average marginal cost, fuel consumption and CO2 emissions
  per technology.
- `summary_by_cluster.csv` — generation, demand, net export and RES share
  per region (uses human-readable names from `cluster_names_k{N}.csv`).
- `summary_by_line.csv` — per inter-cluster line: flow, mean loading, hours
  above 80/90/99% capacity, capacity added.
- `summary_by_fuel.csv` — total fuel consumption (in CAMMESA reporting units:
  dam³, t, m³) and CO2 emissions per fuel code, with share of total emissions.
- `fuel_by_generator.csv` — per-generator fuel consumption and emissions for
  full traceability.
- `new_capacity.csv` — capacity built per generator and per line.

The fuel and emissions accounting reads physical constants
(heating values, reporting units, CO2 emission factors per IPCC / Tercer BUR)
from `fuel_properties.yaml`. For each thermal generator with an
`efficiency_fuel` attribute, the script computes:

```
fuel_physical = generation_MWh × (860 / efficiency × 1000) / heating_value
fuel_report   = fuel_physical × physical_to_report_factor
co2_emissions = fuel_report × co2_emission_factor
```

Generators with no fuel code (renewables, hydro, nuclear) are reported with
NaN in the fuel/CO2 columns.

**Configurable parameters (CLI):**
- `--scenario` — scenario name (default: `2035_BAU`)
- `--k` — cluster level (default: 10)
- `--solver` — `highs` / `gurobi` / `cplex` (default: `highs`)

**Cluster naming:** if `data/network_500kv/clusters/cluster_names_k{N}.csv`
exists with columns `cluster_id` and `name`, region names replace raw
`cluster_X` labels in all summary CSVs and console output. Falls back to raw
ids if the file is missing.
